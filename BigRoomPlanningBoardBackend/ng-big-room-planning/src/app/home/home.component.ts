...
import { PlannedPeriod, Session } from '../client';
import { PeriodNamePipe } from '../pipes/period-name.pipe';
import { getCurrentSession, getPlannedPeriods } from '../store/app.selectors';
...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.periods$ = this.store$.pipe(
      ...
      map((periods) => {
        return create(periods)
          .sort((x) => x.startDay.getTime(), 'desc')
          .toArray();
      })
    );

    this.hasNoPeriods$ = this.periods$.pipe(map((x) => !x?.length));
  }
  ...
  onLogout() {
    this.createEventService.deleteSession(this.connectionService.session);
    this.connectionService.leaveSession();
  }
}
